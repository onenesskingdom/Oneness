...
// FIX: The LiveSession type is not exported from the SDK.
// This uses TypeScript's utility types to infer the session type
// from the `ai.live.connect` method.
...
export const useGeminiLive = (isSpeakerOn: boolean) => {
  ...
  useEffect(() => {
    // Mute/unmute logic for the speaker
  ...
  const startSession = useCallback(async () => {
    ...
    try {
      ...
      const sessionPromise = ai.live.connect({
        // FIX: Updated model name to the recommended model for real-time audio conversations.
        ...
        callbacks: {
          ...
          onmessage: async (message: LiveServerMessage) => {
            ...
            // FIX: Handle conversation interruptions from the server.
